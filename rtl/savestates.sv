...
// extra master cycle instead of racing that capture. The SA1 mapping keeps
// the combinational path: its ROM strobe rises in the same cycle its address
// changes, so a registered address would miss the launch edge entirely.
reg  [23:0] rom_addr_r;
reg         ss_rom_ovr_r;
always @(posedge clk) begin
	// savestate.bin ROM
